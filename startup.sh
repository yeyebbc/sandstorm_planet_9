...
Insurgency/Binaries/Linux/InsurgencyServer-Linux-Shipping Oilfield?Scenario=Scenario_Refinery_Checkpoint_Insurgents?Lighting=Day?MaxPlayers=8 -Port=8001 -QueryPort=8003 -hostname="[第九行星] ISMC 真實動作 地圖包" -log -LogCmds="LogGameplayEvents Log" -Mods -ModList=Mods.txt ModDownloadTravelTo=Conflict?Scenario=Scenario_Conflict_Checkpoint_Insurgents?Lighting=Day -GameStatsToken=E83EFA9BDCD34829A8B18298C420E9BA -NoEAC -MapCycle=MapCycle.txt -mutators=ISMCarmory_Legacy,ForceRankISMC,RealisticMovementHardcore,MapVoteLabels,SlowCaptureTimes,FPLegsPlus,HardcoreHUD,ThrowableMines
